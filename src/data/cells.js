...
// ---- DCB02: 27 ô (dữ liệu thật từ Excel) ---------------------------------
// Dựng theo bố cục sơ đồ bản vẽ:
//   - Cột trái dọc: TM-01..07 (7 ô)
//   - 4 cột giữa × 2 hàng: trên TM-08..11, dưới TM-12..15 (8 ô)
//   - Khối phải: hàng trên NV-16..19, hàng dưới NV-24..27 (đánh phải→trái),
//     cột phải-cùng dọc NV-20..23.
{
  const byOto = new Map(DCB02_RAW.map((r) => [r.o, r]));
  const push = (oto, x, y, w, h) => {
    const raw = byOto.get(oto);
    if (raw) cells.push(makeDumpCell(raw, 'DCB02', 'dcb02', x, y, w, h));
  };
